...
if (isset($_POST["edit_std"])) {
  ...
  `reg_Charges`='$reg_Charges',`sportsCharg`='$sportsCharg', `discount`='$discount' WHERE `students`.`id` = '$edit_id'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST["edit_std"])) {
  ...
  `doj`,`add_fees`,`tutionFee`,`annualCharg`,`institute`,`place_of_birth`,`lab_charges`,`img_dir`,`reg_Charges`,`sportsCharg`,`discount`)
   VALUES 
   ('$std_name','$gr_no','$nationality','$religion','$gender','$dob','$class','$session','$address','$fname','$fcnic','$foccupation'
    ...
    '$annualCharges','$institute','$placeOfBirth','$labCharges','$img_dir','$reg_Charges','$sportsCharg','$discount')";
  $result = mysqli_query($connection, $sql);
